...
# curl -sL "https://www.cursor.com/api/download?platform=linux-x64&releaseTrack=stable" | jq -r '.commitSha,.version'
BUILD_ID="a1f686545fd0ce8917bbd2449f733551a9bce420"
...
src_install() {
	...
	if ! use kerberos && [[ -d ${CURSOR_HOME}/resources/app/node_modules/kerberos ]]; then
		rm -r "${CURSOR_HOME}/resources/app/node_modules/kerberos" || die
	fi

	# Remove foreign-arch ripgrep binaries to avoid QA soname warnings (path varies by release)
	local ripgrep_vendor="${CURSOR_HOME}/resources/app/extensions/cursor-agent/dist/claude-agent-sdk/vendor/ripgrep"
	if [[ -d ${ripgrep_vendor} ]]; then
		if use amd64; then
			rm -r "${ripgrep_vendor}/arm64-linux" || die
		elif use arm64; then
			rm -r "${ripgrep_vendor}/x64-linux" || die
		fi
	fi
