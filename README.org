...
*** Rust packages (crate tarball)
    ...
    After the bump PR copies the ebuild, from overlay root:

    #+begin_src bash
      $ bb bump:crates --pkg-atom games-util/open-steam-controller --version 1.2.0
    #+end_src

    That regenerates md5-cache metadata, downloads upstream sources, runs
    =pycargoebuild -c=, uploads the crate tarball to release =1.0.0=, then
    refreshes Manifest and README. Needs =pycargoebuild=, =pkgdev=, and =gh=.
    Useful flags: =--dry-run=, =--skip-upload=, =--skip-manifest=.
