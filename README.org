...
*** Chicken eggs (source tarball)
    ...
    #+begin_src bash
      $ bb bump:eggs
      $ bb bump:eggs --pkg-atom dev-chicken/srfi1 --version 0.5.1
    #+end_src

    Source order: overlay =files/= (optional) -> =~/.cache/chicken-install= ->
    =--from-henrietta= (=chicken-install -r=). Needs =pkgdev= and =gh= for
    upload/Manifest. Flags: =--dry-run=, =--skip-upload=, =--skip-manifest=.
